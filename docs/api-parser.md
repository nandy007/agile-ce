...
1.v-html与v-text指令用法相似，主要区别是v-html不支持字符串拼接，只能赋值一个变量
...
如果使用的样式位于组件内，则可以通过<code>v-context="funcName"</code>来设置。
...
目前Agile CE的指令扩展可以通过<code>$.vm.addParser(rules)</code>实现。
